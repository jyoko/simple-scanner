...
class WPProbe {
  ...
  constructor(config={}) {
    ...
    this.baseUri     = utilities.makeFirstAndLastSlash(config.baseUri || '');
  ...
  // Needs path (empty string is fine for path)
  ...
  checkUris() {
    ...
    function handleResponse(err, resp, body) {
      ...
      if (this.uriIndex >= uriData.onlyWpStringIndex) {
        ...
        if (this.containsWpStrings(body)) {
          this.foundWp = this.foundWp ? true : 'maybe';
        }
      }
