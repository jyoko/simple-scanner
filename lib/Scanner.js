...
class Scanner extends EventEmitter {
  ...
  _makePortList(config, start=1, end=1000, interval=1) {
    ...
    if (Array.isArray(config.ports)) {
      ...
      portList = config.ports.slice().map(Number).filter(_=>!isNaN(_)&&_>0&&_<65536);
      ...
      if ( this.start < 1   || this.end < 1      ||
           this.end > 65535 || this.interval < 1 ||
           this.end < this.start) {
      ...
      portList = new Array(Math.ceil((this.end-this.start+1)/this.interval));
      for(var i=0, l=portList.length; i<l; i++) {
        portList[i] = this.start+(this.interval*i);
      }
